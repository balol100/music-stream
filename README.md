...
- 🎭 **Mood AI** — Claude Haiku בוחר פלייליסט מותאם לפי מצב רוח (8 פריסטים) או טקסט חופשי
- 📋 **פלייליסטים** — צור, שמור, שתף ציבורית, נגן באמצעות Supabase (לא דורש התחברות, מזהה מכשיר אנונימי)
- 🌟 **פלייליסטים פופולריים** — מסודרים לפי מספר השמעות
- 🗂️ סינון לפי תקופות (כולל "ישראלי") וסגנון
...
- ⌨️ קיצורי מקלדת: `←`/`→` ניווט · `R` אקראי · `S` ערבוב · `F` מועדף · `/` חיפוש
...
המשתמש בוחר מצב רוח (או כותב טקסט חופשי) → קליינט קורא ל-edge function `music-mood` ב-Supabase →
ה-function שולח את הקטלוג + הבקשה ל-Claude Haiku (`claude-haiku-4-5-20251001`) → Claude
מחזיר JSON עם שם פלייליסט, תיאור, ו-5–8 שירים עם סיבה לכל אחד → הקליינט מציג כרטיס AI ייעודי
ומאתחל תור ניגון. **Rate limit:** 5 בקשות ביום לכל IP, נספרות ב-Postgres.
